...
class ApiCalling {
  ...
  ///postApi calling
  static Future<ApiHandler> postApi(
      {required String uri, required Map<String, dynamic> body}) async {
    try {
  ...
  ///putApi calling
  static Future<ApiHandler> putApi(
      {required String uri, required Map<String, dynamic> body}) async {
    try {
      http.Response data = await http.put(Uri.parse(uri), body: jsonEncode(body),);
      if (data.statusCode == 200) {
        return ApiHandler.withSuccess(data);
      } else {
        return ApiHandler.withError(data);
  ...
  static Future<ApiHandler> deleteApi({required String uri}) async {
    try {
      http.Response data = await http.delete(Uri.parse(uri),);
      if (data.statusCode == 200) {
        return ApiHandler.withSuccess(data);
      } else {
        return ApiHandler.withError(data);
      ...
      switch (apiError.statusCode) {
        case 400:
          error = Errors(
              code: 400,
              message:
                  "He server cannot or will not process the request due to something that is perceived to be a client error");
        case 401:
          error = Errors(
              code: 401,
              message:
        ...
        case 422:
          ...
          ErrorResponse errorResponse =
              ErrorResponse.errorHandler(apiData["errors"] ?? {});
          if (errorResponse.errorList.isNotEmpty) {
            ...
            error = Errors(
                ...
                message: apiData['message'] ??
                    "Failed to load data - status code: ${apiError.statusCode}");
          }
          break;
        case 500:
          error = Errors(
